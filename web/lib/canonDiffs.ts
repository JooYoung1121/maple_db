...
export const CANON_DIFFS: Record<string, CanonDiffEntry> = {
  ...
  "edelstein.data-provenance": {
    id: "edelstein.data-provenance",
    path: "/battle-mage",
    subject: "에델슈타인 데이터 판본",
    status: "version",
    mapleland: "메이플랜드는 빅뱅 이전 KMST(2010-05-25) 공개 데이터를 기준으로 자체 설정했습니다 (공지 명시). 몹 레벨은 9/7 공지 확정값, 경험치·일부 HP는 커뮤니티 실측값입니다.",
    original: "추출 가능한 원작 데이터는 빅뱅 이후 GMS(v95) 클라이언트뿐이라 이 지역은 시점이 꼬여 있습니다 — 실제로 몹 레벨 7종·경험치(약 2.3배)가 어긋나 교체했습니다.",
    note: "맵 구조·스폰 배치·마릿수도 GMS(빅뱅 직후) 참고값입니다. 갱도4=경비로봇L, 위험한 너구리 소굴=라칸 등 일부는 실측으로 검증됐지만 나머지 배치는 메랜과 다를 수 있어 실측 제보로 계속 갱신합니다.",
