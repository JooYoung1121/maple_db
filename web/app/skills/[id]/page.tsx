    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6 mt-3">
        ...
        <div className="flex gap-2 mt-2">
          ...
          {skill.job_branch && <span className="text-xs px-2 py-0.5 bg-gray-100 rounded">{skill.job_branch}</span>}
        ...
        <div className="grid grid-cols-2 gap-4 mt-6">
          <div><span className="text-sm text-gray-500">마스터 레벨</span><p className="font-medium">{skill.master_level ?? "-"}</p></div>
          <div><span className="text-sm text-gray-500">직업</span><p className="font-medium">{skill.job_class}</p></div>
        </div>
        {skill.description && (
          <div className="mt-4">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  <th className="px-4 py-3 text-left font-semibold text-gray-600 w-20">레벨</th>
                  <th className="px-4 py-3 text-left font-semibold text-gray-600">효과</th>
                </tr>
              </thead>
              <tbody>
                {levelData.map((ld, i) => (
                  <tr key={i} className={`border-b border-gray-100 ${i % 2 === 0 ? "bg-white" : "bg-gray-50/50"}`}>
                    ...
                    <td className="px-4 py-2 text-gray-700">{ld.effect}</td>
                  </tr>
                ))}
              </tbody>
            </table>
