      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6 mt-3">
        <h1 className="text-2xl font-bold">
          {(() => {
            ...
            return kr ? (
              <>{kr.name_en} <span className="text-lg font-normal text-gray-500">({quest.name})</span></>
            ) : quest.name;
          })()}
        </h1>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mt-6">
          <div><span className="text-sm text-gray-500">레벨 요구</span><p className="font-medium">{quest.level_req || "-"}</p></div>
          <div><span className="text-sm text-gray-500">시작 NPC</span><p className="font-medium">{quest.npc_start || "-"}</p></div>
          <div><span className="text-sm text-gray-500">종료 NPC</span><p className="font-medium">{quest.npc_end || "-"}</p></div>
        </div>
        {quest.description && (
          <div className="mt-4">
