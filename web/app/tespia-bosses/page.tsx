const sourceLinks = [
  ...
  {
    label: "메이플노트 클래식 — 발록 드랍테이블 (과거버전 데이터)",
    href: "https://xn--o80b01o9mlw3kdzc.com/monster_detail/8830000",
  },
  {
    label: "나무위키 빅토리 메이플 (공식 클래식 월드의 카오스 보스 보상)",
    href: "https://namu.wiki/w/%EB%B9%85%ED%86%A0%EB%A6%AC%20%EB%A9%94%EC%9D%B4%ED%94%8C",
  },
  {
    label: "메이플플래닛 2026.06.18 패치노트 (카오스 혼테일 도입)",
    href: "https://mapleplanet.co.kr/board/update/616",
  },
  {
    label: "메이플플래닛 2026.06.23 패치노트 (카오스 자쿰 패턴)",
    href: "https://mapleplanet.co.kr/board/update/623",
  },
];
...
const bosses = [
  {
    ...
    patterns: [
      ...
      "[플래닛 참고] 빅뱅 직전 시점 서버 메이플플래닛은 6/23 패치로 카자쿰 몸체에 유혹·물약 봉인·언데드·방향키 전환·어둠의 그림자 패턴을 추가했습니다 — 카자쿰 최초 출시 당시엔 없었고 빅뱅 이후 원작에 추가된 패턴이라는 게 플래닛 공식 설명입니다. 메랜이 초기형/후기형 중 무엇을 쓸지는 미확정입니다.",
    ],
    drops: {
      db: "현재 사이트 DB 드롭 0건 (카오스 자쿰의 투구는 아이템 DB에 등록됨)",
      note:
        "외부 현대 자료에는 카오스 자쿰 투구/분노한 자쿰 장비류가 언급되지만 메랜 테스피아 드롭으로 확정하지 않았습니다. 참고: 공식 클래식 월드 '빅토리 메이플'에서는 카오스 자쿰 보상이 '카오스 자쿰의 투구 3개 고정'으로 알려져 있습니다(나무위키 — 메랜과 다를 수 있음).",
      table: [] as string[][],
    },
    strategy:
  ...
  },
  {
    ...
    drops: {
      db: "현재 사이트 DB 드롭 0건",
      note:
        "과거버전(프리빅뱅) 데이터에는 카오스 혼테일 드랍테이블 자체가 존재하지 않음을 확인했습니다(메이플노트 클래식 기준 드랍 데이터 공백) — 메랜 2.0의 카혼테일 드롭은 자체 세팅일 가능성이 큽니다. 공식 선례 두 가지를 참고로 병기합니다. ① 클래식 이벤트 월드 '빅토리 메이플': 카오스 혼테일의 목걸이 3개 고정 보상(나무위키). ② 메이플플래닛(빅뱅 직전 시점 서버, 6/18 공식 패치노트): 보상 '카오스 혼테일의 목걸이', 도전 조건 Lv.110+, 1일 1회, 제한시간 2시간 30분, 운명의 바퀴 10회 제한, 방어율 80%(플래닛 자체 상향), 성·화·빙·뇌·독 속성 데미지 감소. 두 서버 모두 밸런스·시점이 메랜과 달라 그대로 신뢰하면 안 되며, 메랜 확정 정보는 출시 후 실측으로 채웁니다.",
      table: [] as string[][],
    },
    strategy:
  ...
  },
  {
    ...
    drops: {
      db: "현재 사이트 DB 드롭 87건: 타임리스/리버스 장비 풀세트(모자·한벌옷·신발·장갑·전 직업 무기), 타임리스 이어링, 시간의 돌, 시간 조각, 엘릭서류",
      note:
        "과거버전 데이터(메이플노트 클래식)도 리버스 무기를 핑크빈 0.5% 드랍으로 기재하고 있어 사이트 DB와 교차 일치합니다. 블랙빈 마크, 핑크빛 성배 등 현대 자료 항목은 메랜 드롭으로 검증 전입니다.",
      table: [] as string[][],
    },
    strategy:
  ...
  },
  {
    ...
    drops: {
      ...
      note:
        "획득 경로 ① 처치 후 보상맵 오브젝트 파괴: '발록의~' 접두 80·90제 무기(업그레이드 9회), 베인 무기 시리즈, 혼돈의 주문서(60%), 백의 주문서(1%), 발록 신발 확률 획득 ② 발록의 가죽조각을 '지하로 내려가는 길' NPC 수상한 남자에게 신발·주문서로 교환(비율 미확인). 발록 주문서: 스탯+2/HP·MP+30/이속+3/점프+5/명중·회피+5/방어+10 계열 30% 11종 + 석양의 발록 주문서 5%(올스탯+4·물마방+14·명중회피+4·이속점프+4·HP/MP+40). 아래 확률표는 과거버전 메이플 데이터(메이플노트 클래식 기준 74종) — 메랜 2.0 적용 여부·드롭률은 미확정입니다.",
      table: [
        ["소비류 21종 (엘릭서·파워엘릭서·이슬·음식·귀환서 등)", "각 35%", "회복/잡화"],
        ["이터널 불릿", "5%", "건슬링거 탄환"],
        ["백의 주문서 1% / 석양의 발록 주문서 5%", "각 0.9%", "주문서"],
        ["'발록의~' 접두 무기 42종 (80·90제, 전 직업)", "각 0.09%", "예: 발록의 백혈귀·페어프로즌·마기코라스"],
        ["베인 무기 6종 (윙즈·스워드·바이터·폴암·롱보우·슈터)", "각 0.09%", "무기"],
        ["발록의 털가죽 신발", "0.09%", "Lv.68 올스탯+2"],
        ["발록의 가죽조각", "0.06%", "NPC 교환 재료"],
        ["혼돈의 주문서 60%", "0.05%", "주문서"],
      ] as string[][],
    },
    strategy:
                ...
                <div className="border-2 border-edge p-3 text-sm leading-6">
                  ...
                  {boss.drops.table.length > 0 && (
                    <div className="mt-3 overflow-hidden border-2 border-edge">
                      ...
                      {boss.drops.table.map(([name, rate, note]) => (
                        <div
                          key={name}
