...
const bosses = [
  ...
  },
  {
    ...
    drops: {
      db: "현재 사이트 DB 드롭 0건",
      note:
        "과거버전(프리빅뱅) 데이터에는 카오스 혼테일 드랍테이블 자체가 존재하지 않음을 확인했습니다(메이플노트 클래식 기준 드랍 데이터 공백) — 메랜 2.0의 카혼테일 드롭은 자체 세팅일 가능성이 큽니다. 가장 가까운 공식 선례는 클래식 이벤트 월드 '빅토리 메이플'로, 카오스 혼테일 보상이 '카오스 혼테일의 목걸이 3개 고정'으로 알려져 있습니다(나무위키). 커스텀 서버(메이플플래닛)에도 카혼테일의 목걸이 드랍 정보가 있으나 밸런스·시점이 달라 참고만 해야 하며, 메랜 확정 정보는 출시 후 실측으로 채웁니다.",
      table: [] as string[][],
    },
    strategy:
