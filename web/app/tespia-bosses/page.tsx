...
const bosses = [
  ...
  },
  {
    ...
    dataWarning:
      "사이트 DB에는 발록 본체가 여러 형태로 나뉘어 있습니다(Lv.105 HP 428만/998만, Lv.56~60 HP 206만~321만 등 — 형태·난이도 변형 추정). 마왕발록(9101003) 단일 레코드는 스펙 미수집 상태입니다. 메랜 2.0에 적용될 버전·수치는 7/16 출시 후 실측 확인이 필요합니다.",
    parts: [
      ["발록 본체 (형태별)", "4,280,000 / 9,980,000 HP", "Lv.105 데이터 기준"],
      ["하위 형태", "2,059,200 ~ 3,210,000 HP", "Lv.56~84 형태 변형"],
      ["입장 위치", "저주받은 신전 지하 — 발록의 무덤", "신전의 밑바닥에서 NPC 무영"],
    ],
    patterns: [
      "입장 후 약 3분간 봉인 해제 연출로 체력이 줄지 않습니다 — 단 일정 데미지를 누적하지 못하면 추방된다는 제보가 있어 꾸준히 공격해야 합니다.",
      "전투 중 발록의 팔 2개를 함께 타격할 수 있는 광역기가 효율적이며, 진행에 따라 팔이 사슬로 봉인되고 이후 머리를 집중 공격하는 구조입니다.",
      "1/1 공격(HP·MP를 1로), 약 1분 주기의 공격반사(피격 시 큰 피해), 언데드화 디버프(물약 효과 절반·힐 피격)가 핵심 위험 요소입니다. 힐 사용 직업은 언데드화 타이밍에 특히 주의해야 합니다.",
      "외부 공략의 레벨 제한·스킬 제한(1·2차만 사용 등) 수치는 후대 KMS 패치 기준이 섞여 있어, 메랜 2.0 원정대 규칙은 출시 후 확인이 필요합니다.",
    ],
    drops: {
      db: "현재 사이트 DB 드롭 0건 (8830000번대 발록 기준)",
      note:
        "출시 패치노트(KMS 1.2.70)·커뮤니티 종합: 발록의 가죽신발(Lv.58)/털가죽 신발(Lv.68), 이터널 불릿(Lv.110), '발록의~' 접두 80·90제 무기(업그레이드 9회), 베인 무기 시리즈(소드/윙즈/롱보우/폴암/바이터/슈터), 발록의 가죽(NPC '수상한 남자'에게 신발·발록 주문서 교환), 혼돈의 주문서, 순백의 주문서. 메랜 테스피아 드롭으로는 미확정입니다.",
    },
    strategy:
