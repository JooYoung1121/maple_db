...
PRESETS: dict[str, tuple[str, list[int], str]] = {
    ...
    "bowmaster":   ("신궁",         [OVERALL["archer"], 1452057], "alert"),               # 엔가우(활)
    "marksman":    ("표사",         [OVERALL["archer"], 1462050], "alert"),               # 블랙뷰티(석궁)
    "nightlord":   ("나이트로드",   [OVERALL["thief"], 1472068], "alert"),                # 람피온(클로)
    "shadower":    ("섀도어",       [OVERALL["thief"], 1332073], "stand1"),                # 페스카즈(단검)
    "archmage":    ("아크메이지",   [OVERALL["mage"], 1372044], "stand1"),                 # 엔릴 티어(완드)
    "bishop":      ("비숍",         [OVERALL["mage"], 1382057], "alert"),                 # 에아스 핸드(스태프)
    "viper":       ("바이퍼",       [OVERALL["pirate"], 1482013], "alert"),               # 용아주조(너클)
    "captain":     ("캡틴",         [OVERALL["pirate"], 1492013], "alert"),               # 드래곤 세인트(건)
